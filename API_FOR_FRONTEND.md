...
4. **READY** → Предложение принято (все проголосовали или создатель нажал «Принять»). Корт и время зафиксированы. Создатель может выбрать тренера (по доступности на слот) и назначить команды.
5. **BOOKED** → Бронь создана (проверки: корт и тренер свободны). Доля каждого участника считается как при обычной брони (слоты корта, абонемент, прайм-тайм, скидка). Ждём оплату от каждого.
6. **PAID** → Все оплатили (доля за слот + личные услуги). Бронь подтверждена.
...
| POST | `lobby/<id>/book/` | Создать бронь (только READY, команды назначены). Доля каждого считается как при обычной брони: слоты корта, абонемент, прайм-тайм, скидка, тренер. Проверки: корт и тренер свободны. Ответ: `booking_id`, `court_total`, `coach_total`, `total`, `players` (у каждого: `court_share`, `membership_used`, `is_paid`). |
...
| POST | `lobby/<id>/pay-share/` | Оплатить свою долю. Body: `payment_method` (KASPI / CARD / CASH). Списывается доля за слот (корт+тренер+прайм) при сумме > 0 (в т.ч. доплата за прайм при абонементе) и личные услуги. Когда все оплатили — статус лобби PAID, бронь CONFIRMED. |
| **POST** | **`lobby/<id>/cancel-booking/`** | Отмена брони лобби (создатель или персонал). Лобби в статусе BOOKED или PAID, бронь в будущем. Каждому участнику возвращаются потраченные деньги (REFUND по доле + доп. услугам) и часы абонемента (если использовал). Ответ: `refunds` (user_id, amount), `hours_returned` (user_id → часы). Ограничение по времени: как у обычной отмены (CANCELLATION_HOURS). |
...
### Ответ POST lobby/<id>/book/ (создание брони)

При успехе (201) в ответе приходят:

- **booking_id** — id созданной брони
- **court_total**, **coach_total** — стоимость корта и тренера за весь слот
- **total** — суммарная цена брони (сумма персональных долей участников)
- **players** — массив по каждому участнику: `user_id`, `user_name`, `team`, `court_share` (доля к оплате: корт+тренер+прайм), `membership_used`, `is_paid` (true если доля 0 и абонемент использован)

Возможные ошибки при POST book/: корт занят; тренер уже занят в это время; время в прошлом; не назначены команды; статус не READY.

### Ответ POST lobby/<id>/cancel-booking/ (отмена брони лобби)

При успехе (200): `status`, `refunds` — список `{ user_id, amount }` по участникам, которым вернули деньги; `hours_returned` — объект `user_id → часы` для участников, которым вернули часы абонемента. Ошибки: не создатель/не персонал; нет брони; статус не BOOKED/PAID; бронь в прошлом; отмена позже чем за CANCELLATION_HOURS (для не-персонала).
