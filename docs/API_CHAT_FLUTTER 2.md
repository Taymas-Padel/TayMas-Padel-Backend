...
**Почему это быстро:**
- Запрос `?after=<id>` возвращает 0–N сообщений за ~20 мс (один SQL-запрос по индексу).
...
**Поля `companion`** — собеседник (не ты). Всегда один объект.  
**`last_message`** — `null` если диалог только создан и сообщений нет.  
...
**Когда вызывать:** при нажатии "Написать" в профиле друга. Получишь `conversation.id` и сразу открывай экран диалога.
...
|----------|-----|-------------|----------|
| `after`  | int | — | ID последнего полученного сообщения. Вернёт только id > after |
...
[
  ...
  },
  {
    ...
    "is_read": false,
    "created_at": "2026-02-23T15:31:45+05:00"
  }
]
```
...
Помечает все непрочитанные сообщения **от собеседника** как прочитанные.
...
- При получении новых сообщений в открытом диалоге (в каждом цикле polling).

---

### 6. Общий счётчик непрочитанных
...
## Ошибки WebSocket (Событие type: "error")

| Код ошибки (`code`) | Описание | Действие |
|---------------------|----------|----------|
| `validation_error`  | Некорректные данные (пустой текст, >4000 символов, неверный `request_id`) | Показать пользователю |
| `bad_format`        | Неверный формат JSON пакета (отсутствуют `type`, `payload`, `v`) | Проверить код приложения |
| `rate_limit_exceeded` | Превышен лимит событий (флуд контроль) | Подождать перед повторной отправкой |
| `server_error`      | Внутренняя ошибка сервера | - |

**Лимиты (Rate Limits) WebSocket:**
- Отправка сообщений (`message.send`): макс. 20 сообщений в минуту
- Отметки прочитанности (`message.read`): макс. 5 раз в 10 секунд
- Статус печати (`typing.start`/`stop`): макс. 5 раз в 5 секунд
- Максимальный размер пакета (кадра WebSocket): **10 КБ** (при превышении сервер разорвет соединение с кодом `4009`)
