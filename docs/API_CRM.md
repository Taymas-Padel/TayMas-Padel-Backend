...
| Роль | Описание |
|------|----------|
| **RECEPTIONIST** | Ресепшн: клиенты, брони, оплаты, абонементы, расписание, финансы, QR. |
| **ADMIN** | Всё то же + дашборд директора, управление кортами, услугами, типами абонементов, акциями, новостями. |

Эндпоинты с пометкой **(только ADMIN)** доступны только пользователям с ролью `ADMIN`. Остальные CRM-эндпоинты доступны и ресепшн, и админу.
