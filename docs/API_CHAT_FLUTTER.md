...
Бэкенд предоставляет **REST API**. Для быстрой доставки сообщений используй **short polling** с параметром `after` — это забирает только новые сообщения и работает очень быстро.
